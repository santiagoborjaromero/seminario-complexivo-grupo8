...
if __name__ == "__main__":
    ...
    # Procesando Movies
    df_movie, movie_source, unique_genres = procesar_movie(dict_df["movie.csv"])
    print(type(df_movie))
    
    # Procesando Tags
    df_tags_general, tags = procesar_tags(dict_df["tag.csv"])
    print(type(df_tags_general))
    exit(0)
    
    # Procesando links
    df_links = procesar_link(dict_df["link.csv"])
    print(type(df_links))
